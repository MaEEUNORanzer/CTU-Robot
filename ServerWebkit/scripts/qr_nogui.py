...
class QRTcpServer:
    ...
    def accept_client(self):
        ...
        try:
            ...
            self.send_to_client("READY: 二维码检测服务器已就绪，等待SCAN命令")
    ...
    def process_command(self, command):
        ...
        if command.upper() == "SCAN":
            if not self.scanning:
                ...
                self.center_align_count = 0
                print(f"[INFO] 开始扫描任务")
                self.send_to_client("START: 开始扫描任务，每0.1秒检测一次")
            ...
            self.send_to_client(f"STATUS: {status}, 中心对齐次数: {self.center_align_count}")
    ...
    def scanning_task(self):
        ...
        try:
            ...
            if detections:
                ...
                # 发送到客户端
                self.send_to_client(f"QR_DETECTED: 偏移={offset:+d}px ({offset_cm:+.2f}cm) {direction}, 对齐={'是' if is_aligned else '否'}")
                ...
                if is_aligned:
                    ...
                    if self.center_align_count >= self.required_center_count:
                        ...
                        self.send_to_client(f"COMPLETE: 已达到连续 {self.required_center_count} 次中心对齐，扫描任务完成")
                        self.scanning = False
                        self.center_align_count = 0
                else:
                    # 重置中心对齐计数
                    ...
                    if offset > 0:
                        adjust_msg = "向右调整"
                        # result = rc.move_right(distance=((offset_cm/100)), speed=0.2)
                        # print(f"向右移动结果: {result}")
                        result = rc.move_backward(distance=0.3, speed=0.3)
                        print(f"结果: {result}")

                        time.sleep(3)
                    else:
                        adjust_msg = "向左调整"
                        # result = rc.move_left(distance=((offset_cm/100)), speed=0.2)
                        # print(f"向左移动结果: {result}")
